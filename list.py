...
_wsl_base_path = Path(
    "/mnt/d/project/cnctd/data/textures/textures_td_srgb/DATA/ART/TEXTURES/SRGB/TIBERIAN_DAWN"
)
_win_base_path = Path(
    "c:d\project\cnctd\data\textures\textures_td_srgb\DATA\ART\TEXTURES\SRGB\TIBERIAN_DAWN"
)
_base_path = None
    ...
    if name.upper().endswith(from_suffix):
        with Image(filename=name) as read_image:
            with read_image.clone() as write_image:
                ...
                write_image_name = str(
                    _output_path.joinpath(Path(name).stem + "." + to_suffix)
                )
